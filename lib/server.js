    ...
    app.get('/', function (req, res) {
     if(req.query.cmd == undefined) {
        ...
        command.on('error', function(error) {
            console.log(self.tool + ' Error: ' + error);
...
server.listen = function(tool) {  
    ...
    self.tool = tool;
    var bs = self.bs;     
    bs.on('err', function(err) {
            console.log('caught bs error: ' + err);
         })
    bs.on('connection', function(client) {
      console.log('connected')
      self.client = client;
      client.on('err', function(err) {
        console.log('caught client error: ' + err);
     })
      client.on('stream', function(stream,options) { 
         ...
         if(options.event == 'run') {            
            ...
            command.on('error', function(error) {
                console.log(self.tool + ' Error: ' + error);
                stream.error(self.tool + ' Error: ' + error);
            })
