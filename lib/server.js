...
server.init = function(port, opts) {
    ...
    app.get('/status', function(req, res){
        ...
        res.send(query.callback + '({"status": "running"});');
    });
    ...
    app.get('/', function (req, res) {
        ...
        var query = urlParser(req.url).query;
        ...
        if(query.cmd == undefined) {
            ...
            if(query.binary) query.encoding = 'binary';
            query.protocol = query.protocol || 'http';
            self.debug = query.debug || false;
            ...
            var cachePath = self.getCachePath(self.config.cacheDir, query);
            self.canRunNow(null, cachePath, function(canRun, cacheReadStream) {
                // initialie command
                var cmdInitialized = command.init(self.tool, res, query, {'cachePath': cachePath, timeout: self.config.timeout });
                if (canRun) {
                    // Initialize command
                    if ( cmdInitialized ) {
                        // if cached
                        if (cacheReadStream) {
                            ...
                            if(query.encoding && query.encoding != 'binary') cacheReadStream.setEncoding(query.encoding);
    ...
    bs.on('connection', function(client, options) {
        ...
        client.on('stream', function(stream,options) {
            ...
            if(options.event == "clientConnected") {
                ...
                console.log('connectionID = ' + options.connectionID);
                console.log('bs.command ids = ' + Object.keys(bs.commands));
            ...
            if(options.event == 'run') {
                ...
                self.canRunNow(client.domain, cachePath, function(canRun, cacheReadStream) {
                    // initialie command
                    var cmdInitialized = command.init(self.tool, stream, params, {'cachePath': cachePath, timeout: self.config.timeout});
                    if (canRun) {
                        // Initialize command
                        if ( cmdInitialized ) {
                            // if cached
