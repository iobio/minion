...
server.init = function(port) {
    ...
    app.get('/', function (req, res) {
        if(req.query.cmd == undefined) {
            ...
            if ( self.canRunNow() ) {
                if(command.init(self.tool, res, req.query)) {
...
server.listen = function(tool) {  
    ...
    bs.on('connection', function(client, options) {        
        ...
        console.log('c.domain = ' + client.domain); 
        client.commands = [];           
        client.on('stream', function(stream,options) {              
            ...
            if(options.event == 'run') {                    
                ...
                // check if command can be run now or should be put in the queue                    
                if ( self.canRunNow(client.domain) ) {                
                    // Initialize command
                    if (command.init(self.tool, stream, params)) {                        
                        // Run command                        
                        self.getServerAddress(function(serverAddress) {
                            self.runningCommands[command.id] = command;                            
...
server.getRequestDomain = function() {
...
server.canRunNow = function(requestDomain) {
    if (this.serverDebug)
    ...
    var mcc = this.config.maxConcurrentCommands;    
    if(mcc == undefined || mcc == null)
        return true;    

    // check if requesting domain is listed as a domain not to be queued    
    if (this.config.noQueueDomains.indexOf(requestDomain) != -1)
        return true

    // check if we are over the max concurrent commands
    if (Object.keys(this.runningCommands).length < this.config.maxConcurrentCommands)
        return true;
    else
        return false;
}
