...
cmd.prototype.init = function(tool, stream, params, opts) {
    ...
    for( var i=0; i < rawArgs.length; i++) {
        ...
        if ( arg.match(/^http%3A%2F%2F\S+/) || arg.match(/^ws%3A%2F%2F\S+/) || arg.match(/^https%3A%2F%2F\S+/) || arg.match(/^wss%3A%2F%2F\S+/) ) {
            ...
            self.minions.push( {
                url: decodeURIComponent(arg),
                fifoPath: fifoPath,
                fifoStream: self._createFifoStream(fifoPath)
            });

            // replace arg with fifo
            args.push(fifoPath);
...
cmd.prototype.run = function(clients, serverAddress) {
    ...
    Promise.all( self.minions.map(function(d) { return d.fifoStream} )).then( function(fifoStreams) {

        // overwrite promises with fifostreams
        self.minions.forEach(function(m) {
            fifoStreams.forEach(function(fs) {
                if (m.fifoPath == fs.path)
                    m.fifoStream = fs;
            })
        })
        ...
        if(self.params.parseByLine || self.params.format != undefined) {
            ...
            // DON'T PIPE. MUST USE 'DATA' EVENT.
            // piping doesn't pull the data through the fifo, causing the buffer to fill up and the stream to pause
            // so must use 'data' event
            // prog.stdout.pipe(stream);
            prog.stdout.on('data', function(d) { stream.write(d); })
            // var fs = require('fs');
            // var ws = fs.createWriteStream('samoutput.bam');
            // prog.stdout.pipe(ws);
        ...
        self.minions.forEach(function(minion,i) {
            ...
            if (parsed.isClient) {
                ...
                self.emit('createClientConnection', {'id': connectionId, 'serverAddress': serverAddress, 'argPos': i});
    ...
    return new Promise(function(resolve, reject) {
        mkdirp('./fifos/', function (err) {
            ...
            fifoprog.on('exit', function() {
                var fifoStream = fs.createWriteStream(fifoPath);
