...
cmd.prototype.init = function(tool, stream, params) {      
...
cmd.prototype.run = function(clients, serverAddress) {    
    ...
    if(self.params.parseByLine || self.params.format != undefined) {
        ...
        // var fs = require('fs');
        // var ws = fs.createWriteStream(stream.guid + '.txt')
        // prog.stdout.pipe(process.stdout);   
        // if (self.cmdArgs.debug) {
          // var fs = require('fs');
          // var ws = fs.createWriteStream('output.gz')
          // prog.stdout.pipe(ws);
        // }
    }
