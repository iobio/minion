...
cmd.prototype.init = function(tool, stream, params, opts) {
    ...
    if (cmd != undefined && cmd != 'undefined') rawArgs = cmd.match(/(?:[^\s"]+|"[^"]*")+/g)
        ...
        self.minions.forEach(function(minion) {
            if (minion.progStreamType == 'fifo') {
                fifoStreams.forEach(function(fifostream) {
                    if (minion.fifoPath == fifostream.path)
...
cmd.prototype.end = function() {
    ...
    // Kill running spawn progs
    self.progs.forEach(function(prog) {
        if(prog) {
            prog.kill('SIGINT');
            if (prog.exitCode != 0 ) {
    ...
    self.runners.forEach(function(r) { r.end() });
}
