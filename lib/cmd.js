...
cmd.prototype.run = function(clients, serverAddress) {
    ...
    if (self.opts.timeout != undefined)
        setTimeout(function() {
            if (self.prog.exitCode == null)
                self.emit('log', 'command timed out after ' + self.opts.timeout + 'ms')
            self.end();
        }, self.opts.timeout);
...
cmd.prototype.end = function() {
    ...
    // kill prog
    if(this.prog) {
        this.prog.kill('SIGINT'); // Kill it gently
...
cmd.prototype.kill = function() {
    ...
    // kill prog
    if(this.prog) {
        this.prog.kill('SIGKILL'); // Kill it harshly
        if (this.prog.exitCode != 0 ) {
            this.emit('log', 'command prematurely ended. Cleaning up ...')
            this.deleteCache();
        }
